...
# Кроме того, добавь ему два новых атрибута через конструктор — precondition и environment. 
# Тип данных для них — строки.
...
# Конструктор суперкласса через super() выглядит так: super().__init__(name). 
# Указывать имя суперкласса не нужно.
# Чтобы данные выводились с новой строки, используй \n. 
# Например, f"\nПредусловие:  {self.precondition}".
# Чтобы вывести все атрибуты суперкласса и добавить ему свои, используй super().
